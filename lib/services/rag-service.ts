...
export async function processRAGQuery(question: string): Promise<RAGResult> {
  try {
    console.log(`🧠 מעבד שאלה: "${question}"`)

    // חיפוש במסמכים רלוונטיים
    const sources = await searchRelevantDocuments(question)
    console.log(`📚 נמצאו ${sources.length} מקורות רלוונטיים`)

    if (sources.length === 0) {
      console.log("⚠️ לא נמצאו מקורות רלוונטיים, משתמש בתשובה כללית")
      return {
        answer:
          "מצטער, לא מצאתי מידע רלוונטי לשאלתך במאגר המידע שלי. אנא נסה לנסח את השאלה בצורה אחרת או פנה לאתר פיקוד העורף הרשמי.",
        sources: [],
        usedFallback: true,
      }
    }

    // הכנת הקונטקסט מהמקורות
    const context = sources.map((source) => `מקור: ${source.title}\nתוכן: ${source.content}`).join("\n\n")

    console.log(`📝 הכנת תשובה עם ${context.length} תווים של קונטקסט`)

    // יצירת תשובה עם AI
    const { text } = await generateText({
      model: openai("gpt-4o"),
      prompt: `
אתה עוזר חירום ישראלי מומחה המבוסס על מידע מפיקוד העורף.
ענה על השאלה הבאה בעברית, בהתבסס על המידע שסופק.
...
3. אם המידע לא מספיק, אמר זאת בבירור
4. תן תשובה מדויקת ומועילה
5. אל תמציא מידע שלא קיים במקורות
...
תשובה:`,
    ...
    console.log(`✅ תשובה נוצרה: ${text.substring(0, 100)}...`)
    ...
    console.error("💥 שגיאה בעיבוד RAG:", error)

    return {
      answer: "מצטער, אירעה שגיאה טכנית. אנא נסה שוב מאוחר יותר.",
      sources: [],
      usedFallback: true,
...
interface DocumentSource extends RAGSource {
  content: string
}

async function searchRelevantDocuments(question: string): Promise<DocumentSource[]> {
  try {
    console.log(`🔍 מחפש מסמכים רלוונטיים לשאלה: "${question}"`)

    // יצירת embedding לשאלה
    const { embedding } = await generateEmbedding(question)

    // חיפוש במסמכים עם similarity search
    ...
    if (!data || data.length === 0) {
      console.log("📭 לא נמצאו מסמכים רלוונטיים")
      return []
    }

    console.log(`📚 נמצאו ${data.length} מסמכים רלוונטיים`)
    ...
    return data.map((doc: any) => ({
      ...
      storage_path: doc.storage_path,
      content: doc.plain_text || doc.summary || "",
    }))
  } catch (error) {
    console.error("💥 שגיאה בחיפוש מסמכים:", error)
    return []
...
async function generateEmbedding(text: string): Promise<{ embedding: number[] }> {
  try {
    // זוהי פונקציה פשוטה - במציאות תשתמש ב-OpenAI embeddings
    // כרגע נחזיר embedding דמה
    console.log("🔢 יוצר embedding לטקסט")

    // TODO: להחליף עם OpenAI embeddings אמיתי
...
export async function saveChatMessage(
  ...
  try {
    ...
    console.error("💥 שגיאה כללית בשמירת הודעה:", error)
    throw error
  }
}
