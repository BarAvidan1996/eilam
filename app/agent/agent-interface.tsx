...
export default function AgentInterface() {
  ...
  const executeNext = async () => {
    ...
    try {
      ...
      // Special handling for recommend_equipment - use AI recommendations API
      let response
      if (execution.tool.id === "recommend_equipment") {
        ...
        try {
          ...
          if (!response.ok) {
            console.warn("AI recommendations API failed, falling back to execute API")
          ...
          console.error("Error using AI recommendations API:", error)
          // Fall back to regular execute API
        }
      }

      // Regular execution for other tools or if AI recommendations failed
  ...
  const renderEquipmentRecommendations = (result: any) => {
    const recommendations = result.result.recommendations
    const isPersonalized = result.result.isPersonalized
    const familyProfile = result.result.familyProfile

    // Handle structured recommendations (new format from AI recommendations API)
      ...
      case "equipment_recommendations":
        return (
          <div className="space-y-3">
            ...
            <div className="bg-primary/5 p-4 rounded-lg border border-primary/20">
              {renderEquipmentRecommendations(result)}
